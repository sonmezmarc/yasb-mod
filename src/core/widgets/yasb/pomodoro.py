...
class PomodoroWidget(BaseWidget):
    ...
    def show_menu(self):
        ...
        # --------- NEW: Edit Durations Button ---------
        edit_button = QPushButton("Edit Times")
        edit_button.setCursor(QCursor(Qt.CursorShape.PointingHandCursor))
        edit_button.setProperty("class", "button edit")
        edit_button.clicked.connect(self._show_edit_dialog)
...
class CircularProgressWidget(QWidget):
    ...
    def _show_edit_dialog(self):
        dialog = QDialog()
        dialog.setWindowTitle("Edit Durations")
        ...
        work_input = QSpinBox()
        work_input.setRange(1, 120)
        work_input.setValue(int(self._work_duration / 60))

        # Break duration
        break_label = QLabel("Break Duration (minutes):")
        break_input = QSpinBox()
        break_input.setRange(1, 60)
        break_input.setValue(int(self._break_duration / 60))

        # Save button
        save_button = QPushButton("Save")
        save_button.clicked.connect(lambda: self._update_durations(work_input.value(), break_input.value(), dialog))

        # Add widgets
        layout.addWidget(work_label)
        layout.addWidget(work_input)
        ...
        dialog.exec()
    def _update_durations(self, work_min, break_min, dialog):
